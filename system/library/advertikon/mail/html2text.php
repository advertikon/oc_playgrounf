...
class html2text
{
    ...
    protected $replace_with_callback = array(
		'/<h[123][^>]*>(.*?)<\/h[123]>/i',           // H1 - H3
		'/<h[456][^>]*>(.*?)<\/h[456]>/i',           // H4 - H6
    ...
    function _convert()
    {
        ...
        $text = preg_replace_callback( '/<h[123][^>]*>(.*?)<\/h[123]>/i', function( $m ) {
        	return strtoupper( "\n\n{$m[1]}\n\n" );
        }, $text );

        $text = preg_replace_callback( '/<h[456][^>]*>(.*?)<\/h[456]>/i', function( $m ) {
        	return ucwords( "\n\n{$m[1]}\n\n" );
        }, $text );

        $text = preg_replace_callback( '/<strong[^>]*>(.*?)<\/strong>/i', function( $m ) {
        	return strtoupper( $m[1] );
        }, $text );

        $text = preg_replace_callback( '/<b[^>]*>(.*?)<\/b>/i', function( $m ) {
        	return strtoupper( $m[1] );
        }, $text );

        $text = preg_replace_callback( '/<a [^>]*href="([^"]+)"[^>]*>(.*?)<\/a>/i', function( $m ) use( $self ) {
        	return $self->_build_link_list( $m[1], $m[2] );
        }, $text );

        $text = preg_replace_callback( '/<th[^>]*>(.*?)<\/th>/i', function( $m ) {
        	return strtoupper( "\t\t{$m[1]}\n" );
        }, $text );
